                  ...
                  Padding(
                    ...
                    child: ElevatedButton(
                      onPressed: (() {
                        ...
                        String? doorId = "8OYTJM1xcxgGyMUiLRKV";
                        if (id == null) {
                          snackBar(context, "ID not found");
                        }
                        if (!_accessibleTimeRange) {
                          snackBar(context,
                              "Door access only available within 30 minutes of meeting start time");
                        }
                        if (!FirestoreService
                            .guestMap[id]!.completedHealthScreen) {
                          snackBar(context,
                              "Health Screen must be completed before gaining building access");
                        }
                        if (FirestoreService.guestMap[id]!.isSick) {
                          snackBar(
                              context, "Access is not granted due to symptoms");
                        }
                        // Code to set door and guest variable in database
                        _fs.guestCollection.doc(id!).update(
                            {"unlockToken": false, "unlockedDoor": true});
                        _fs.doorCollection
                            .doc(doorId)
                            .update({"locked": false});
                      ...
                      style: ElevatedButton.styleFrom(
                          backgroundColor: (unlockDoor(widget.meeting))
                              ? Colors.blue[500]
                              : Colors.grey),
                  ...
                  Padding(
                    ...
                    child: ElevatedButton(
                      onPressed: (() {
                        ...
                        if (!_accessibleTimeRange) {
                          snackBar(context,
                              "Health Screen access only available within 30 minutes of meeting start time");
                        }
                        Navigator.push(
                            context,
                            MaterialPageRoute(
                      ...
                      style: ElevatedButton.styleFrom(
                          backgroundColor: (_accessibleTimeRange)
                              ? Colors.blue[500]
                              : Colors.grey),
